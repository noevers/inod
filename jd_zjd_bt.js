...
=====================Loon================
[Script]
cron "50 0 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_zjd_bt.js, tag=京东赚京豆补团
====================Surge================
*/
...
const openTuanNum = $.isNode() ? (process.env.OPEN_TUAN_NUM ? process.env.OPEN_TUAN_NUM : 2):2;
$.cookiesArr = [];
$.tuan = null;

!(async () => {
  if (!getCookies()) return;
  for (let i = 0; i < openTuanNum; i++) {
    $.firstCookie = $.cookieArr[i];
    if ($.firstCookie) {
      const userName = decodeURIComponent(
        $.firstCookie.match(/pt_pin=(.+?);/) && $.firstCookie.match(/pt_pin=(.+?);/)[1],
      );
...
async function helpFriendTuan(body,userNameSecond) {
  return new Promise(resolve => {
    ...
    delete body['time'];
    $.get(taskTuanUrlZL("vvipclub_distributeBean_assist", body), async (err, resp, data) => {
      try {
        if (err) {
...
function getUserTuanInfo() {
  ...
  return new Promise(resolve => {
    $.get(taskTuanUrlKT('distributeBeanActivityInfo', body), async (err, resp, data) => {
      try {
        $.log(data);
...
function createTuan(id) {
  ...
  return new Promise(resolve => {
    $.get(taskTuanUrlKT('vvipclub_distributeBean_startAssist', body), async (err, resp, data) => {
      try {
        $.log(data);
        data = JSON.parse(data);
        if (data.success) {
...
function taskTuanUrlKT(function_id, body = {}) {
  return {
...
function taskTuanUrlZL(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(JSON.stringify(body))}&appid=swat_miniprogram&osVersion=5.0.0&clientVersion=3.1.3&fromType=wxapp&timestamp=${new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 * 1000}`,
    headers: {
      "Accept": "*/*",
