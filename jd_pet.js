...
[task_local]
#东东萌宠
50 6-18/6 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_pet.js, tag=东东萌宠, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdmc.png, enabled=true

=================================Loon===================================
[Script]
cron "50 6-18/6 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_pet.js,tag=东东萌宠

===================================Surge================================
东东萌宠 = type=cron,cronexp="50 6-18/6 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_pet.js

====================================小火箭=============================
东东萌宠 = type=cron,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_pet.js, cronexpr="50 6-18/6 * * *", timeout=3600, enable=true
...
async function jdPet() {
  try {
    ...
    if (iinitPetTownRes && nitPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
...
async function energyCollect() {
  ...
  if (response && response.resultCode && response.resultCode === '0') {
  ...
  if (response.code === '0' && response.resultCode === '0' && response.message === 'success') {
    ...
    if (foodAmount - 100 >= 10) {
      for (let i = 0; i < parseInt((foodAmount - 100) / 10); i++) {
        ...
        if (feedPetRes && feedPetRes.resultCode && feedPetRes.resultCode == 0 && feedPetRes.code == 0) {
          console.log('投食成功')
        }
			$.wait(2000);
  ...
  if (res && res.code === '0' && res.resultCode === '0') {
    ...
    if (res.result.masterHelpPeoples && res.result.masterHelpPeoples.length > 0) {
      ...
      res.result.masterHelpPeoples.map((item, index) => {
        if (item && index === (res.result.masterHelpPeoples.length - 1)) {
...
async function slaveHelp() {
  ...
  for (let code of newShareCodes) {
    ...
    if (response && response.code && response.code === '0' && response.resultCode === '0') {
