...
===========小火箭========
抢京豆 = type=cron,script-path=https://raw.githubusercontent.com/noevers/inod/master/jd_rob_bean.js, cronexpr="0 0 * * *", timeout=3600, enable=true
*/
const $ = new Env('抢京豆');
...
async function getHelpCode(){
  return new Promise(resolve => {
    const options = {
      url: `https://api.m.jd.com/client.action?functionId=signBeanGroupStageIndex&body=%7B%22monitor_refer%22%3A%22%22%2C%22rnVersion%22%3A%223.9%22%2C%22fp%22%3A%22-1%22%2C%22shshshfp%22%3A%22-1%22%2C%22shshshfpa%22%3A%22-1%22%2C%22referUrl%22%3A%22-1%22%2C%22userAgent%22%3A%22-1%22%2C%22jda%22%3A%22-1%22%2C%22monitor_source%22%3A%22bean_m_bean_index%22%7D&appid=ld`,
      headers: {
        "accept": "*/*",
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          data = JSON.parse(data)
          if (data.code==0) {
		  console.log(`京东账号:${$.UserNameSecond}帮助结果：`+data.data.helpToast)
          }else{
            console.log(data)
          }
        }
